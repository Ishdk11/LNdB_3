...
def extractCube(scan,spacing,xyz,cube_size=40,cube_size_mm=40):
...
def get_the_plot(lnd,rad,finding):
    ...
    axs[0].imshow(scan[int(ctr[2])],cmap='gray')
    axs[0].axis('off')
    axs[1].imshow(mask[int(ctr[2])],cmap='gray')
    axs[1].axis('off')
    plt.show()
    ...
    axs[0,0].imshow(scan_cube[int(scan_cube.shape[0]/2),:,:],cmap='gray')
    axs[0,0].axis('off')
    axs[1,0].imshow(mask_cube[int(mask_cube.shape[0]/2),:,:],cmap='gray')
    axs[1,0].axis('off')
    axs[0,1].imshow(scan_cube[:,int(scan_cube.shape[1]/2),:],cmap='gray')
    axs[0,1].axis('off')
    axs[1,1].imshow(mask_cube[:,int(mask_cube.shape[1]/2),:],cmap='gray')
    axs[1,1].axis('off')
    axs[0,2].imshow(scan_cube[:,:,int(scan_cube.shape[2]/2)],cmap='gray')
    axs[0,2].axis('off')
    axs[1,2].imshow(mask_cube[:,:,int(mask_cube.shape[2]/2)],cmap='gray')  
    axs[1,2].axis('off')  
    plt.show()  
...
def GetGlcmFeatures(img):
    result = greycomatrix(img, distances = [1], angles = [0,45,90,135], levels = 4, symmetric=True)
...
nodules = csvlines[1:]
#ax = ['x','y','z']
with open('glcm_features.csv', 'w', newline = '') as file:
    writer = csv.writer(file)
    writer.writerow(['lndbId', 'RadId', 'FindingId', 'contrast','energy','dissimilarity','homogeneity','correlation','ASM'])
    for n in nodules:
        ...
        contrast,energy,dissimilarity,homogeneity,correlation,asm = GetGlcmFeatures(img)
        writer.writerow([lnd,rad,finding,contrast,energy,dissimilarity,homogeneity,correlation,asm])
        print('done')
    # img_x, img_y, img_z = get_the_plot(lnd,rad,finding)
    # print(GetGlcmFeatures(img_x))

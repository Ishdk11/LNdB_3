# -*- coding: utf-8 -*-
"""
Created on Mon Jul 27 12:41:41 2020
...
def GetFeatures(img):
    real, imag = gabor(img, 7)
    result = greycomatrix(real, distances = [1], angles = [0,45,90,135], levels = 65, symmetric=True)
...
nodules = csvlines[1:]
ax = ['x','y','z']
with open('gabor_with_glcm_features.csv', 'w', newline = '') as file:
    writer = csv.writer(file)
    writer.writerow(['lndbId', 'RadId', 'FindingId', 'contrast','dissimilarity','energy','homogeneity','correlation','ASM'])
    for n in nodules:
        ...
        contrast,energy,dissimilarity,homogeneity,correlation,asm = GetFeatures(img)
